...
export default function MultimediaGallery({
  ...
  // Refrescar lista de imágenes desde el servidor
  const refrescarImagenes = async () => {
    ...
    try {
      ...
      const imagenesActualizadas: Imagen[] = data.imagenes.map((img: any) => ({
        name: img.nombre,
        url: img.url,
        size: img.tamano || 0,
        createdAt: img.fecha,
        lastModified: img.actualizado || img.fecha,
      }));

      setImagenes(imagenesActualizadas);
      setSeleccionadas(new Set()); // Limpiar selección
      toast.success('Lista de imágenes actualizada');
    } catch (error) {
      console.error('Error al refrescar:', error);
      toast.error('Error al refrescar la lista de imágenes');
    } finally {
      setCargando(false);
    }
  };
  ...
  const handleSubirImagenes = async (files: FileList) => {
    ...
    try {
      ...
      if (exitosas > 0 && fallidas === 0) {
        ...
        // Refrescar después de 1 segundo para obtener metadatos completos
        setTimeout(() => refrescarImagenes(), 1000);
